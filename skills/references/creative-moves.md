...
each with a cheap falsification test. Dead routes go to `ideas.md` with the reason, so they are not retried.

## Route format (required)
...
- **M61 Evolutionary program search** over constructions with an exact scorer (`python -m harness evolve`); mine the elite
  population for structure, then prove the structure.
...
- A route without a cheap falsification test is not a route.
- Do not investigate a route whose falsification test failed; log it (`ideas.md`, status `dead`, reason) and move on.
- Parallel persona provers (combinatorialist / analyst / algebraist / experimentalist) should each take a *different* route;
  the judge runs a tournament and cross-pollinates the survivors.
